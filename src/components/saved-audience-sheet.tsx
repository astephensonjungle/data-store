	...
	return (
		<Sheet open={open && !!savedAudienceId} onOpenChange={onOpenChange}>
			<SheetContent className="w-full max-w-xl text-sm">
				<SheetHeader>
					<SheetTitle>{isLoading ? <Skeleton className="h-6 w-48" /> : audience?.name}</SheetTitle>
				</SheetHeader>
				<div className="flex h-full flex-1 flex-col gap-4 p-6 pt-0">
						...
						<div className="mt-2 flex flex-col divide-y">
							<div className="flex justify-between py-2">
								...
								<div className="text-foreground">
									{isLoading ? <Skeleton className="h-5 w-20" /> : new Intl.NumberFormat("en-US").format(audienceCount)}
								</div>
							</div>
							<div className="flex justify-between py-2">
					...
					<div className="flex justify-end">
						<Button>Activate Audience</Button>
					</div>
				</div>
			</SheetContent>
		</Sheet>
